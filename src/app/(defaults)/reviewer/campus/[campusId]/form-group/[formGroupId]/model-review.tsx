...
function ModalReviewCampus({ payload }: { payload: { questionId: string; campusUserId: string; formGroupId: string; variableId: string; year: string } }) {
  ...
  const createReview = api.reviewer.campus.createReviewSurveyCampus.useMutation({
    onMutate() {
      void Swal.fire({
        title: 'Mohon Tunggu!',
        text: 'Sedang menyimpan hasil review',
    ...
    async onSuccess() {
      await Promise.all([
        utils.reviewer.campus.getSelectedSurveyCampusReview.invalidate({
          campusUserId,
          formGroupId,
          variableId,
        }),
        utils.reviewer.campus.getModalSelectedSurveyCampusReview.invalidate({
          campusUserId,
          formGroupId,
          variableId,
          year,
      ...
      void Swal.fire({
        title: 'Berhasil!',
        text: 'Hasil review berhasil disimpan',
        icon: 'success',
        customClass: { container: 'sweet-alerts' },
      });
      setShowModal(false);
